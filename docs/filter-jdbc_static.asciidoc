...
<6> Uses values for user fields if the user can not be found in the users database. 
In which case the <<plugins-{type}s-{plugin}-tag_on_default_use>> tags list is used to mark the event.
