...
be used.
NOTE: 
Before using `fetch_size` check your driver's manual about how to configure server side cursors.
For example, in MySQL there is a connection string property named `useCursorFetch`, in PostgreSQL
there isn't any specific flag but auto commit must be disabled, resultset must be of forward 
only (which is the default) and fetch size must be a integer greater than zero. 
