...
The JDBC input plugin leverages the "sequel" library to query databases through their JDBC drivers.
However, the implementation of drivers may vary, potentially leading to unexpected behavior in connection pool management.

===== Unable to reuse connection

Stale connections may return to the pool, causing the plugin reuse closed connections. Sybase is affected by this issue.

One solution is to enable <<plugins-{type}s-{plugin}-jdbc_validate_connection>> and set <<plugins-{type}s-{plugin}-jdbc_validation_timeout>> to a shorter interval than the <<plugins-{type}s-{plugin}-schedule>>.
